...
def readtxt(filename, row, col):
    ...
    for line in open(filename):
        for i in line.split():
            result.append(i)
    data = numpy.zeros((row, col), float)
    k = 0
    for i in range(row):
        for j in range(col):
            data[i][j] = result[k]
...
# )
#
# hht_extractor = HHTFeatureExtractor(
#     fs=48000,
#     win_length_seconds=0.04,
#     hop_length_seconds=0.02
# )
...
for fold in db.folds():
    ...
    for item_id, audio_filename in enumerate(db.audio_files):
        ...
        feature_txtfilename = dcase_util.utils.Path(
            path=audio_filename
        ).modify(
            path_base=os.path.join(os.path.abspath('.'), 'systems', 'features'),
            filename_extension='.txt'
        )

        feature_origin = readtxt(feature_txtfilename, 28, 500)


        # 特征的存储的名称
        feature_filename = dcase_util.utils.Path(
            path=feature_txtfilename
        ).modify(
            path_base=os.path.join('systems', 'ASC_GMM_SIMPLE', 'features'),
            filename_extension='.cpickle'
        )

        # Extract features and store them into FeatureContainer, and save it to the disk（此处有修改）
        dcase_util.containers.FeatureContainer(
            data=feature_origin,
            time_resolution=os.path.join('systems', 'ASC_GMM_SIMPLE', 'features')
        ).save(
...
for audio_filename in db.audio_files:
    ...
    #     filename=feature_filename,
    #     data=hht_extractor.extract(audio.data),
