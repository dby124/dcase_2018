...
#
# DCASE2018开发数据集，log-mel energies 特征和GMM分类器
#
# ===================================================================================
...
log.title('Acoustic Scene Classification / GMM')
...
db = dcase_util.datasets.TUTUrbanAcousticScenes_2018_DevelopmentSet(data_path='datasets')
...
extractor_mel = dcase_util.features.MelExtractor(
    fs=48000,
    win_length_seconds=0.04,
    hop_length_seconds=0.02
)

extractor_mfcc = dcase_util.features.MfccStaticExtractor(
    fs=48000,
    win_length_seconds=0.04,
    hop_length_seconds=0.02,
    n_mfcc=14
...
mfcc_delta_extractor = dcase_util.features.MfccDeltaExtractor(
    fs=48000,
    win_length_seconds=0.04,
    hop_length_seconds=0.02,
    n_mfcc=14
...
mfcc_acceleration_extractor = dcase_util.features.MfccAccelerationExtractor(
    fs=48000,
    win_length_seconds=0.04,
    hop_length_seconds=0.02,
    n_mfcc=14
...
zrc_extractor = dcase_util.features.ZeroCrossingRateExtractor(
    fs=48000,
    win_length_seconds=0.04,
    hop_length_seconds=0.02
)

rmse_extractor = dcase_util.features.RMSEnergyExtractor(
    fs=48000,
    win_length_seconds=0.04,
    hop_length_seconds=0.02
)

centroid_extractor = dcase_util.features.SpectralCentroidExtractor(
    fs=48000,
    win_length_seconds=0.04,
    hop_length_seconds=0.02
)
...
for audio_filename in db.audio_files:
    ...
    origin_features = numpy.vstack((extractor_mel.extract(audio.data), extractor_mfcc.extract(audio.data)))
    ...
    features = dcase_util.containers.FeatureContainer(
        ...
        data=mfcc_delta_extractor.extract(audio.data),
        # data=numpy.vstack((extractor_mel.extract(audio.data), extractor_mfcc.extract(audio.data))),
        time_resolution=extractor_mel.hop_length_seconds
    ).save()
